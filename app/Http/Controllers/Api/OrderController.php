    ...
    public function store(Request $request)
    {
        ...
        foreach ($shops_cart_items as $shop_id => $shop_cart_item_ids) {
            ...
            foreach ($shop_cart_items as $cartItem) {
                ...
                $product_details = Product::find($cartItem->product_id);

                // Serial number maintain & stock quantity start
                $i=0;
                $sold_serial_nos = array();
                $left_serial_nos = array();
                $serial_nos = json_decode($product_details->serial_no);
                if(!empty($serial_nos)){
                    foreach($serial_nos as $si_no){
                        if($i<$cartItem->quantity){
                            array_push($sold_serial_nos,$si_no);
                        }
                        if($i>=$cartItem->quantity){
                            array_push($left_serial_nos,$si_no);
                        }
                        $i++;
                    }
                }

                $product_details->serial_no = json_encode($left_serial_nos);
                $product_details->stock_quantity = $product_details->stock_quantity-$cartItem->quantity;
                $product_details->save();
                // Serial number maintain & stock quantity end
                ...
                $orderDetail = OrderDetail::create([
                    ...
                    'actual_price' => $product_details->highest_price,
                    'tax' => $itemTax,
                    'total' => ($itemPriceWithoutTax+$itemTax)*$cartItem->quantity,
                    'quantity' => $cartItem->quantity,
                    'prod_serial_num' => json_encode($sold_serial_nos),
                ]);
